...
const ShopListData = () => {
    ...
    const fetchCategories = async () => {
        ...
        setLoadingProducts(true); // Set loading products to true as well
        try {
          let res = await ProductCategory(headers);
          let categories = res.results;
          setCategoriesData(categories);
          fetchProducts(categories); // Fetch products after setting categories
        } catch (error) {
      ...
      const fetchProducts = async (categories) => {
        try {
          ...
          Promise.all(promises).then((results) => {
            ...
            setLandingData(data);
            setLoadingProducts(false); // Set loading products to false after all products have been fetched
          });
        } catch (error) {
          console.error('Error fetching products:', error);
          setLoadingProducts(false); // Ensure loading products is set to false even if there is an error
        }
      };
                      ...
                      <div className="container">
                        ...
                        <div className="">
                          {products[key].length === 0 ? (
                            ...
                            <CardData products={products[key]} handleFavList={fetchCategories} />
                          )}
                        </div>
